...
cat ${DIR}/springboot-push-architecture.ascii
set -e
...
helm upgrade -i keda-otel-scaler -nkeda oci://ghcr.io/kedify/charts/otel-add-on --version=v0.1.1 -f ${DIR}/springboot-push-values.yaml
...
kubectl rollout status -n observability --timeout=600s \
  ...
  deploy/grafana
kubectl create cm dashboard -nobservability --from-file=${DIR}/push-grafana-dashboard.json
...
kubectl apply -f <(cat ${DIR}/so-springboot-push.yaml | envsubst)
...
# create traffic
(hey -z 120s http://localhost:8080/ping &> /dev/null)&
